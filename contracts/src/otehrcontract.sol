...
contract MultiOutcomePredictionMarket2 is CoprocessorAdapter {
    ...
    event MarketCreated(uint256 marketId, string question, string[] outcomesOptions);
    event RequestCreated(uint256 requestId, uint256 deadline, bytes inputBytes, address requester);
    event RequestFullfilled(uint256 requestId);
    ...
    mapping(uint256 marketId => uint256[] circulatingShares) public s_circulatingShares;
    mapping(uint256 marketId => Market market) public s_markets;
    mapping(uint256 requestId => Request request) public s_requests;
    mapping(uint256 marketId => uint256[] winningOutcomes) public s_winningOutcomes;
    mapping(uint256 marketId => uint256 prizePool) public s_prizePools;
    ...
    function createMarket(string memory _question, uint256 _deadline, string[] memory _outcomesOptions) public {
        ...
        s_marketId++;
        s_markets[s_marketId] =
            Market({question: _question, outcomesOptions: _outcomesOptions, deadline: _deadline, isResolved: false});
    ...
    function getShares(uint256 _marketId) public view returns (uint256[] memory) {
